...
// Méthodes et pièges en texte brut, pour les données structurées (HowTo + FAQ)
const SEO_FICHES = [
  {
    nom: 'Calculer une dose avec le produit en croix',
    etapes: ['Pose ton tableau : à gauche ce que tu as, à droite ce que tu cherches, en vérifiant que les unités correspondent ligne par ligne.', 'Multiplie en diagonale les deux valeurs qui se font face.', 'Divise par la valeur restante, celle qui est seule dans son coin.'],
    faq: [['Que faire si les unités sont différentes ?', "Si la prescription est en g et le flacon en mg, convertis d'abord (1 g = 1 000 mg), puis pose le produit en croix."], ['Comment éviter d\'inverser les valeurs ?', 'Assure-toi que mg est avec mg et ml avec ml, sur la même colonne du tableau.'], ['Comment repérer un résultat aberrant ?', 'Si tu trouves 75 ml de sirop pour un enfant, c\'est probablement faux : vérifie toujours l\'ordre de grandeur.']],
  },
  {
    nom: 'Calculer le débit d\'une perfusion en gouttes par minute',
    etapes: ['Repère tes données : le volume en ml, le temps en heures et le type de tubulure (standard 1 ml = 20 gouttes, pédiatrique 1 ml = 60 gouttes).', 'Au numérateur, multiplie le volume par le facteur de gouttes (20 ou 60).', 'Au dénominateur, convertis le temps en minutes (heures × 60), puis divise et arrondis à l\'entier.'],
    faq: [['Combien de gouttes dans 1 ml ?', 'Avec une tubulure standard, 1 ml = 20 gouttes. Avec une tubulure pédiatrique, 1 ml = 60 gouttes.'], ['Quelle est la formule des gouttes par minute ?', 'Débit (gouttes/min) = (volume en ml × 20) ÷ (durée en heures × 60) pour une tubulure standard.'], ['Faut-il arrondir le résultat ?', 'Oui, toujours à l\'entier : une goutte ne se coupe pas en deux. 41,6 devient 42 gouttes/min.']],
  },
  {
    nom: 'Convertir des unités de masse (g, mg, µg)',
    etapes: ['Détermine le sens de la conversion : vers une unité plus petite (g vers mg), on multiplie par 1 000 ; vers une plus grande (µg vers mg), on divise par 1 000.', 'Déplace la virgule de 3 rangs : vers la droite si × 1 000, vers la gauche si ÷ 1 000, en ajoutant des zéros si nécessaire.'],
    faq: [['Combien de mg dans 1 g ?', '1 g = 1 000 mg, et 1 mg = 1 000 µg (aussi noté mcg).'], ['Que veut dire mcg ?', 'mcg est la notation anglo-saxonne du microgramme (µg) : 1 000 mcg = 1 mg.'], ['Comment convertir des µg en mg ?', 'On divise par 1 000 : la virgule se déplace de 3 rangs vers la gauche. 2 500 µg = 2,5 mg.']],
  },
  {
    nom: 'Calculer une masse à partir d\'une concentration en pourcentage',
    etapes: ['Lis le pourcentage : X % signifie X g pour 100 ml (G5% = 5 g de glucose pour 100 ml).', 'Regarde le volume réel du flacon, en convertissant les litres en ml si besoin (1 L = 1 000 ml).', 'Fais un produit en croix : si le volume est de 100 ml, lis directement la valeur.'],
    faq: [['Que signifie NaCl 0,9 % ?', '0,9 % = 0,9 g de chlorure de sodium pour 100 ml, soit 9 g dans une poche d\'un litre.'], ['Que signifie G5% ?', 'G5% = 5 g de glucose pour 100 ml de solution.'], ['Quelle différence entre % et g/L ?', '5 % = 5 g pour 100 ml = 50 g/L : le facteur entre les deux est de 10.']],
  },
]
...
function ThemeDebit({ theme }) {
  return (
    <>
      ...
      {/* Tableau de référence des débits (tubulure standard) */}
      <div className="mb-16">
        <Eyebrow couleur={theme.couleur}>Le tableau de référence (tubulure ×20)</Eyebrow>
        <div className="overflow-x-auto">
          <div className="grid grid-cols-6 rounded-xl overflow-hidden ring-1 ring-black/[0.08] bg-white text-center w-max mx-auto">
            <div className="bg-black/[0.03] p-2.5 px-4 text-[9px] font-extrabold uppercase tracking-wider text-black/35 flex items-center justify-center">Volume</div>
            {['4 h', '6 h', '8 h', '12 h', '24 h'].map(d => (
              <div key={d} className="bg-black/[0.03] p-2.5 px-4 text-[11px] font-extrabold text-black/50">{d}</div>
            ))}
            <div className="bg-black/[0.03] p-2.5 px-4 text-[11px] font-extrabold text-black/50 flex items-center justify-center border-t border-black/[0.06]">500 ml</div>
            {[42, 28, 21, 14, 7].map((v, i) => (
              <div key={i} className="p-2.5 px-4 font-bold text-sm border-t border-l border-black/[0.06]" style={{color: theme.couleur}}>{v}</div>
            ))}
            <div className="bg-black/[0.03] p-2.5 px-4 text-[11px] font-extrabold text-black/50 flex items-center justify-center border-t border-black/[0.06]">1 000 ml</div>
            {[83, 56, 42, 28, 14].map((v, i) => (
              <div key={i} className="p-2.5 px-4 font-bold text-sm border-t border-l border-black/[0.06]" style={{color: theme.couleur}}>{v}</div>
            ))}
          </div>
        </div>
        <p className="mt-3 text-center text-xs font-semibold text-black/40">Débits en gouttes/min, arrondis à l&apos;entier — tubulure standard, 1 ml = 20 gouttes.</p>
      </div>
...
export default function FichesCalculsDoses({ actif }) {
  // Données structurées de la fiche affichée (HowTo + FAQ)
  const seo = SEO_FICHES[actif]
  const jsonLd = [
    {
      '@context': 'https://schema.org',
      '@type': 'HowTo',
      name: seo.nom,
      step: seo.etapes.map((texte, i) => ({ '@type': 'HowToStep', position: i + 1, text: texte })),
    },
    {
      '@context': 'https://schema.org',
      '@type': 'FAQPage',
      mainEntity: seo.faq.map(([q, r]) => ({ '@type': 'Question', name: q, acceptedAnswer: { '@type': 'Answer', text: r } })),
    },
  ]
    ...
    <>
      ...
      <section className="relative px-5 pt-8 pb-5 sm:pb-8 overflow-hidden" style={{background: `linear-gradient(to bottom, #ffffff 0%, ${theme.couleur}0b 160px, ${theme.couleur}0b 100%)`}}>
        ...
        <div className="relative max-w-3xl mx-auto">
          {/* Les 4 interrupteurs de thème : des liens, une URL par fiche (collants au défilement) */}
